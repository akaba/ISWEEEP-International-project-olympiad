...
<html>
<head>
    <title>Where do I-SWEEEP projects come from?</title>
...
<script type="text/javascript">
    ...
    function createMarker(point, address, name1, num) {
        ...
        GEvent.addListener(marker, "click", function() {
            ...
            marker.openInfoWindowHtml("" + name1 + "<br/>" + address+ "<br/>");
        });
        return marker;
    }
...
//map.setCenter(point, 10);
		var newMarker = createMarker(point, place.address, name, no - 1);
            map.addOverlay(newMarker);
		newMarker.openInfoWindowHtml("" + name + "<br/>" + place.address);
        ...
        if (data.length > 1)
		setTimeout("continueFinding()",200);
    }
...
function s() {
    ...
    <%
DatabasePath = "E:\Web\isweeep\app_data\isweeep.mdb"

set conn = Server.CreateObject("ADODB.Connection")
conn.Provider="Microsoft.Jet.OLEDB.4.0"
conn.Open databasepath
set rst = Server.CreateObject("ADODB.RecordSet")
sql="SELECT * from QualifiedInternational" 
rst.open sql,conn

do while not rst.eof


Title1=rst("ProjectTitle")
Title2=Replace(Title1,"""","\""")
Title=Replace(Title2,"'","")
